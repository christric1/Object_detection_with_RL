...
class yolo:
    ...
    def detectImg(self, image: Tensor, targets: Tensor, draw_box=False, img_name="image.png") -> str:
        """
        Args
        ...
        if draw_box:
            ...
            for i, det in enumerate(NMS_out):
                if len(det):
                    ...
                    det[:, :4] = scale_coords(padded_images.shape[2:], det[:, :4], image.shape[1:]).round()

                    # Write results
                    for *xyxy, conf, cls in reversed(det):
                        label = f'{names[int(cls)]} {conf:.2f}'
                        plot_one_box(xyxy, img_cv, label=label, color=colors[int(cls)], line_thickness=1)

            cv2.imwrite(os.path.join(self.save_dir, img_name), img_cv)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='runs\score\exp5', help='source')  # file/folder, 0 for webcam
